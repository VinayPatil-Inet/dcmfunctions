    ...
    function inprocess_review_return_report_individual_view(uploadedfileid,currentstatus,usertype) {
        ...
        if(usertype=="PCMH")
        {
            ...
            else
            {
                ...
                var query = "select *,(select count(*) from uploadedfiles_tbl inner join returnreport_data_tbl on returnreport_data_tbl.uploadedfileid=uploadedfiles_tbl.uploadedfileid where uploadedfiles_tbl.pcmh_id='"+pcmh_id+"' and returnreport_data_tbl.currentstatus='"+currentstatus+"' "+month_forall_q+" "+year_forall_q+" "+search_q_forall+") as totalrec, returnreport_data_tbl.monthname as month_name  FROM uploadedfiles_tbl inner join returnreport_data_tbl on returnreport_data_tbl.uploadedfileid=uploadedfiles_tbl.uploadedfileid where uploadedfiles_tbl.pcmh_id='"+pcmh_id+"' and returnreport_data_tbl.currentstatus='"+currentstatus+"' "+month_forall_q+" "+year_forall_q+" "+search_q_forall+" ORDER BY returnreport_data_id DESC OFFSET "+start+" ROWS FETCH NEXT "+length+" ROWS ONLY" ;
            }
        }
        else
        {
